...
public interface StudentRepository extends JpaRepository<Student, String> {
    ...
    @Query("SELECT NEW com.CareerNexus_Backend.CareerNexus.dto.Temp(s.userId, s.FirstName, s.LastName, s.urls) " + // Assuming resumeUrl is the field
            "FROM Student s " +
            "WHERE s.Year = :year AND s.Department = :department ") // <--- ALL SELECTED FIELDS HERE
    List<Temp> findStudentsByYearAndDepartment(
