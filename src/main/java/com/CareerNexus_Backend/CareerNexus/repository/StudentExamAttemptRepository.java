...
public interface StudentExamAttemptRepository extends JpaRepository<StudentExamAttempt, Long> {
    ...
    /**
     * All attempts for a given assessment round (any status).
     */
    List<StudentExamAttempt> findByAssessmentRound_Id(Long assessmentRoundId);

    /**
     * Attempts for a round filtered by result status (e.g. PASSED / FAILED).
     */
    List<StudentExamAttempt> findByAssessmentRound_IdAndResultStatus(Long assessmentRoundId, String resultStatus);
}